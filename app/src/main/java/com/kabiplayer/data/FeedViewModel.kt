...
import com.kabiplayer.data_models.*
...
class FeedViewModel @Inject constructor() : ViewModel() {
    ...
    fun getFeedPagedList(): LiveData<PagedList<com.kabiplayer.data_models.FeedItem>> {
    ...
    fun fetchData() = viewModelScope.launch(Dispatchers.IO) {
        ...
        val demoTvShowsDeferred = async { discoverDemoTvShows() }
        ...
        val demoTvShows = demoTvShowsDeferred.await()
        ...
        if (popularMovies != null) {
            results.add(
                FeedItem.HorizontalList("Trending Movies",
                    popularMovies.map { it.toMediaMovie() })
            )
        }
        if (demoTvShows != null) {
            results.add(
                FeedItem.HorizontalList(
                    "Today Special",
                    demoTvShows.map { it.toMediaTvShow() },
                    true
                )
            )
        }
    ...
    private suspend fun discoverDemoTvShows(): List<TvShow>? {
        return try {
